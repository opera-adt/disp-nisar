...
GLOBAL_ATTRS = {
    ...
    "title": "OPERA_L3_DISP-NISAR Product",
}
...
COMPRESSED_SLC_TEMPLATE = "compressed_{date_str}.h5"
...
def create_output_product(
    ...
    near_far_incidence_angles : tuple[float, float]
        ...
        If not specified, uses approximate NISAR values of (33.0, 47.0)
    ...
    def _get_start_end_cslcs(files):
        if len(files) == 1:
            ...
            # TODO: Check this, there is no burst id
...
def _create_identification_group(
    ...
    with h5netcdf.File(output_name, "a") as f:
        ...
        _create_dataset(
            ...
            dimensions=(),
            data="https://search.asf.alaska.edu/#/?dataset=OPERA-NI&productTypes=CSLC",
            fillvalue=None,
            description=(
        ...
        _create_dataset(
            ...
            dimensions=(),
            data="[3-10, 30-50, 7-10]",
            fillvalue=None,
            description=(
                "List of [StripMap, ScanSAR and Interferometric wide swath mode]"
                "range resolutions from source L1 NISAR SLCs"
            ),
            attrs={"units": "meters"},
        )
        _create_dataset(
            ...
            description=(
                "List of [StripMap, ScanSAR and Interferometric wide swath mode]"
                "azimuth resolutions from source L1 NISAR SLCs"
            ),
            attrs={"units": "meters"},
        )
        _create_dataset(
...
def _create_metadata_group(
    ...
    with h5netcdf.File(output_name, "a") as f:
        ...
        _create_dataset(
            group=metadata_group,
            name="disp_nisar_software_version",
            dimensions=(),
...
def create_compressed_products(
    ...
    comp_slc_dict : dict[str, list[Path]]
        A dictionary mapping "" to a list of compressed SLC files.
    output_dir : Filename
    ...
    for _, comp_slc_files in comp_slc_dict.items():
        for comp_slc_file in comp_slc_files:
            # Pick out the one that matches the current date
