"""Water mask creation utilities for NISAR data.
...
__all__ = ["create_water_mask", "create_mask_from_distance"]
...
def set_aws_env_from_saml(profile_name="saml-pub", region="us-west-2"):
    ...
    logger.info(f"AWS credentials loaded from profile: {profile_name}")
...
def create_water_mask(
    ...
    if frame_id is not None:
        try:
            ...
            epsg, bounds = opera_utils.get_frame_bbox(frame_id=frame_id)
            t = Transformer.from_crs(epsg, 4326, always_xy=True)
            bbox = t.transform_bounds(*bounds)
        except Exception as e:
            raise ValueError(
                f"Failed to get bbox for frame {frame_id}. "
                "For NISAR, provide bbox directly instead. "
                f"Error: {e}"
            )
