#!/usr/bin/env python3
"""Convert a series of OPERA DISP-NISAR products to a single-reference stack.
...
Usage:
    python align_to_common_reference_date.py single-reference-out/ OPERA_L3_DISP-NI_*.nc
"""
...
class OperaDispFile(BaseModel):
    """Class for information from one DISP-NI production filename."""
    ...
    def from_filename(cls, name: Path | str) -> "OperaDispFile":
        ...
        pattern = re.compile(
            r"OPERA_L3_DISP-NI_F(?P<frame_id>\d{5})_"
            r"(?P<polarization>HH|VV|HV|VH)_"
...
def _parse_datetimes(
    ...
    """Parse the datetimes from OPERA DISP-NI filenames."""
