#!/usr/bin/env python3
"""Rename a DISP-NI NetCDF file to the official OPERA DISP-NI naming convention.
...
def _read_disp_nisar_metadata(nc_file: Path) -> dict:
    """Read the DISP-NI metadata from the NetCDF file.
...
def create_filename(
    ...
    name : str, optional
        PGE (product) name, default 'DISP-NI'.
    ...
    core_filename = (
        f"{project}_{level}_{name}_{frame_str}_{polarization}_"
...
def rename_disp_nisar_file(
    input_file: Path,
    output_dir: Optional[Path] = None,
    version: str = "0.10",
    dry_run: bool = True,
) -> Path:
    """Rename a DISP-NI NetCDF file to the OPERA naming convention.
    ...
    """
    meta = _read_disp_nisar_metadata(input_file)
...
def main(input_files, output_dir, version, dry_run):
    """Rename a DISP-NI NetCDF file to an official OPERA name."""
    for input_file in input_files:
        rename_disp_nisar_file(
