...
echo "docker run --rm -it $image_name disp-nisar --help"
#
echo "To run on a PGE runconfig:"
echo "docker run --user \$(id -u):\$(id -g) -v \$PWD:/work --rm -it $image_name disp-nisar run runconfig.yaml"
# where...
...
#     opera/disp-nisar:latest  # The name of the image to run.
#     disp-nisar runconfig.yaml # The `disp-nisar` command that is run in the container
